  ...
  <div class="flex items-center">
    <div class="flex w-full bg-white rounded shadow">
      <dropdown :auto-close="false"
        class="focus:z-10 px-4 hover:bg-red-100 border-r focus:border-white rounded-l focus:ring md:px-6"
      ...
      <input class="relative px-6 py-3 w-full rounded-r focus:shadow-outline" autocomplete="off" type="text"
        name="search" placeholder="Procurar..." :value="modelValue"
        @input="$emit('update:modelValue', $event.target.value)" />
    </div>
    <button class="ml-3 text-gray-500 hover:text-gray-700 focus:text-red-500 text-sm" type="button"
      @click="$emit('reset')">Resetar</button>
  </div>
</template>
